...
10. Server audio export handles audio clips and its supported effects. Workstation instruments, buses, some effects and automation require browser rendering. Do not silently flatten or bypass an advanced mix. Artwork and alignment are optional; alignment confidence can be poor and timestamps may need correction in Visuals.
11. Live Radio is shared, ephemeral and consumes GPU time until explicitly stopped. Check current Radio before starting or retuning. Its current song continues while the next direction is prepared; no seek, skip or pause. Do not replace another user's station without their instruction.
...
def capabilities_document(protected):
    return {
        ...
        'music':{'engine':'m-a-p/YuE2-3B','candidates':[1,2,4,8],'renderSteps':{'Fast':16,'Standard':32,'Extended':48,'Maximum effort':64},'exactDuration':False,'sampleAccurateInpainting':False,'deterministicVocalCasting':False,'instrumentalAdapter':'instrumental-v1','humExperimental':True},
        ...
        'limits':['Song duration follows composition, not an exact timer.','More acoustic steps are more computation, not a measured quality guarantee.','Duet roles and lyric language are requests; singer identity and pronunciation are not certified.','Lyric alignment may require manual correction.','MCP messages and agent JSON bodies are limited to 4 MiB.'],
    }
